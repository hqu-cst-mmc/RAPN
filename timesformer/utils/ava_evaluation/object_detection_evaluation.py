...
2) Add detection predict_true_result of images sequentially.
3) Evaluate detection metrics on already inserted detection results.
4) Write evaluation predict_true_result into a pickle file for future processing or
   visualization.
...
class ObjectDetectionEvaluator(DetectionEvaluator):
    ...
    def evaluate(self):
        """Compute evaluation predict_true_result.
...
class ObjectDetectionEvaluation(object):
    ...
    """
        ...
        if image_key in self.detection_keys:
            logging.warn(
                "image %s has already been added to the detection predict_true_result database",
                image_key,
            )
            return
