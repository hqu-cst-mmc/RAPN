...
class decoder_fuser(nn.Module):
    def __init__(self, dim, num_heads, num_layers, exe_query_num=2, sync_query_num=3):
        ...
        # querys
        # two query to execution score features
        self.exe_querys = nn.Parameter(torch.zeros(1, exe_query_num, dim))
        # three query to synchronized score features
        self.sync_querys = nn.Parameter(torch.zeros(1, sync_query_num, dim))
        # pos embedding
        self.pos_embed_exe = nn.Parameter(torch.zeros(1, exe_query_num, dim))
        self.pos_embed_sync = nn.Parameter(torch.zeros(1, sync_query_num, dim))
        ...
        # 初始化参数
        trunc_normal_(self.exe_querys, std=.02)
        trunc_normal_(self.sync_querys, std=.02)
        trunc_normal_(self.pos_embed_exe, std=.02)
        trunc_normal_(self.pos_embed_sync, std=.02)

    # q是视频1 step_i的特征,v是视频2 step_i的特征图
    def forward(self, v, q_type='exe_query'):
        # 1-input feature
        B, T, dim = v.shape
        # 2-generate referees query
        if q_type == 'exe_query':
            q_e = self.exe_querys
            q_e = q_e + self.pos_embed_exe
            q_e = self.pos_drop(q_e)
            q_e = q_e.expand(B, -1, -1)
            for _layer in self.model:
                q_e = _layer(q_e, v)
            return q_e
        elif q_type == 'sync_query':
            q_s = self.sync_querys
            q_s = q_s + self.pos_embed_sync
            q_s = self.pos_drop(q_s)
            q_s = q_s.expand(B, -1, -1)
            for _layer in self.model:
                q_s = _layer(q_s, v)
            return q_s
